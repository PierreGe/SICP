...
(library 
  ...
  ;; (while true (display 5))
  (define (analyze-while-old exp)
    (let ((wcond (analyze (while-predicate exp)))
          (wbody (analyze (while-body exp))))
      (lambda (env cnt)
        (wcond env
               (lambda (pred-value)
                 (if (true? pred-value)
                     (wbody env ((analyze-while exp) env cnt))
                     (wbody env cnt)))) )))


  (define (while-predicate-old exp) 
    (cadr exp))
  
  (define (while-body-old exp) 
    (caddr exp))


  (define (analyze-while-toon exp)
    (let ((wcond (while-predicate exp))
          (wbody (analyze-sequence (while-body exp))))
      (lambda (env cnt)
        (define (loop pred-value)
          (if (true? pred-value)
              (wbody env (lambda (ignore) ((analyze wcond) env loop)) )
              (wbody env cnt)))
        ((analyze wcond) env loop))))

  ;; (while true (display 5))
  ;; (while false (display 5))
  (define (analyze-while-old-old exp)
    (let ((wcond (while-predicate exp))
          (wbody (analyze (while-body exp))))
      (define (loop wcond wbody env cnt)
        ((analyze wcond) env
          (lambda(pred-value)
            (if (true? pred-value)
                (wbody env (loop wcond wbody env cnt) )
                (wbody env cnt)))))
      (lambda (env cnt)
        (wbody (loop wcond wbody env cnt)) )))
